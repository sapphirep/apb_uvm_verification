...
(   
  ...
  input   logic [STRB_WIDTH-1:0] i_pstrb,
...
// 8-bit  => addr[ADDR_WIDTH-1:0]                        
// 16-bit => addr[ADDR_WIDTH-1:1], ie., ignore addr[0:0] bits 
// 32-bit => addr[ADDR_WIDTH-1:2], ie., ignore addr[1:0] bits
localparam ADDR_LSB = $clog2(DATA_WIDTH/8)     ;
localparam N_REG    = 2**(ADDR_WIDTH-ADDR_LSB) ;  // Max. no of registers supported in the address space
...
//       ...
//-----------------------------------------------------------------------------------
logic [DATA_WIDTH-1:0] apb_reg[5] ; 

// Read/write errors
logic wr_err, rd_err ;
logic [ADDR_WIDTH-1-ADDR_LSB:0] paddr ;
      ...
      case (state_ff)
         ...
         W_ACCESS : begin             
            ...
            if (req_wr && i_penable) begin
               ...
               case (i_paddr [ADDR_WIDTH-1:ADDR_LSB])
...
// Word address
assign paddr = i_paddr [ADDR_WIDTH-1:ADDR_LSB] ;
...
assign o_pslverr = wr_err | rd_err | (paddr > 4);
